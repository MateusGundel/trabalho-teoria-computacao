...
public class Passo1 {
    ...
    public LinkedList<InstRotulada> instrucoes;

    public LinkedList<InstRotuladaComp> lista = new LinkedList<>();

    public static int contadorRotuloComposto = 1;
    public int contPosAtu = 0;
    StringBuilder texto;

    public InstRotuladaComp iRC = new InstRotuladaComp();
    public InstRotulada instrucaoInicial;
    ...
    public Passo1(LinkedList<InstRotulada> instrucoes) {
    ...
    public int busca_proxima_instrucao(boolean primeiroConjunto, InstRotulada instrucaoInicial2) {
    ...
    public List<InstRotulada> rotulaOperacoesEmOrdemExecucao(List<InstRotulada> instrucoes2) {
        ...
        for (InstRotulada instrucao : instrucoes) {
        ...
        for (Integer indexTesteAtual : listaOrdemExecucaoOperacoes) {
            try {
                InstRotulada instrucao = instrucoes.get(indexTesteAtual.intValue());
                // Se for operacao
     ...
     */
    public LinkedList<InstRotuladaComp> gerar() throws Exception {
        ...
        for (int i = 0; i < instrucoes.size(); i++) {
            InstRotulada instrucao = instrucoes.get(contPosAtu);
            ...
            if (instrucao.getOpTeste().getRotulo() == Rotulo.NENHUM
                ...
                while (iRC.getOpF() == null) {
                    ...
                    InstRotuladaComp iRC2 = buscaOperacao(Rotulo.OPERACAO_FALSO, instrucao, iRC, instrucoes);
    ...
    public String comoString(LinkedList<InstRotuladaComp> lista) {
        texto = new StringBuilder();

        for (InstRotuladaComp iRC : lista) {
    ...
    public InstRotuladaComp buscaOperacao(Rotulo rotuloOp, InstRotulada instrucao, InstRotuladaComp iRC, LinkedList<InstRotulada> instrucoes) throws Exception {

        erro = false; // reseta variavel

        InstRotulada instrucaoEmTeste = null;
